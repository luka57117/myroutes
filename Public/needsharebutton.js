...
(function() {
    ...
    root.options.networks = root.options.networks.toLowerCase().split(',');
		...
		***********************************************/
        ...
        //AL use the original element in case of empty button text
		if (!root.button) {
		  if (root.options.buttonText.length) {
			  root.button = document.createElement('span');
			  root.button.innerText = root.options.buttonText;
			  root.elem.appendChild(root.button);
			}
			else
			  root.button = root.elem;
		}
...
//		      root.dropdown.style.marginLeft = - root.dropdown.offsetWidth / 2 + 'px';
		      root.dropdown.style.marginLeft = - DD_WIDTH / 2 + 'px';
		      break
	   		case 'middleLeft':
...
//		      root.dropdown.style.marginTop = - DD_WIDTH / 2 + 'px';
		      break
	   		case 'middleRight':
